package com.prahlad.aijobportal.jobservice.feign;
 ...
 * authenticated recruiter's {@code companyId}/{@code companyName} when
 * creating a job. Resolved via Eureka using the registered service id
 * ({@code RECRUITER-SERVICE}). Forwards the caller's own
 * already-validated bearer token — does not duplicate authentication or
 * recruiter-management business logic.
 */

package com.prahlad.aijobportal.candidateservice.exception;
...
/**
 * Centralized exception translation for the Candidate Service. Ensures
 * every error response — validation failures, business rule violations,
 * file-upload errors, and unexpected errors alike — uses the same
 * {@link ApiResponse} envelope and never leaks a raw stack trace.
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleMultipartException(Exception ex) {
        ApiError error = ApiError.builder()
                ...
                .message("The uploaded file is missing, malformed, or exceeds the maximum allowed size")
                .build();
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ApiResponse.failure("Invalid file upload", List.of(error)));
    }

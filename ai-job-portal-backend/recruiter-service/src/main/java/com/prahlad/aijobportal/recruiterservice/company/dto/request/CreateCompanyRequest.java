...
/**
 * Request payload for {@code POST /recruiter/companies}. The creating
 * recruiter's identity (email, name) is NOT accepted here — it is
 * fetched authoritatively from Auth Service via Feign at creation time.
 * Registering a company implicitly creates the calling user's
 * {@code Recruiter} profile as the company's owner, so the recruiter's
 * own title/designation/phone are collected in the same request rather
 * than requiring a separate follow-up call.
 */
public record CreateCompanyRequest(
        ...
        @NotNull(message = "Recruiter title is required")
        RecruiterTitle recruiterTitle,

        @Size(max = 150, message = "Recruiter designation must not exceed 150 characters")
        String recruiterDesignation,

        @Pattern(regexp = "^\\+?[0-9\\-\\s()]{7,20}$", message = "Recruiter phone number must be a valid phone number")

package com.prahlad.aijobportal.applicationservice.security.filter;
 ...
 * {@code /api/v1/applications/internal/admin/**} (never routed through
 * the API Gateway) using a shared secret header, per DECISIONS.md
 * ("Internal JWT for Service-to-Service Communication"). Mirrors the
 * identical pattern already established in Auth Service's
 * {@code InternalServiceAuthFilter}, added here so Admin Service can read
 * application statistics without duplicating Application Service's
 * business logic.
 */
@Component
@Slf4j
